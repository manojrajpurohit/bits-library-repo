...
def create_emp():
    try:
        _json = request.json
        door_no = _json['door_no']
        street = _json['street']
        area = _json['area']
        pin_code = _json['pin_code']
        if door_no and street and area and pin_code \
                and request.method == 'POST':
            conn = mysql.connect()
            cursor = conn.cursor(pymysql.cursors.DictCursor)
            sqlQuery = "INSERT INTO address (door_no, " \
                       "street, area, pin_code) VALUES(%s, %s, %s, %s)"
            bindData = (door_no, street, area, pin_code)
...
def emp_details(emp_id):
    try:
        ...
        cursor = conn.cursor(pymysql.cursors.DictCursor)
        cursor.execute(
            "SELECT id, name, email, phone, address FROM emp WHERE id =%s",
            emp_id)
